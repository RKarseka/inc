...
export async function startApp() {
  ...
  const result = await ordersCollection.find({$or:[{productTitle: {$regex: '^IPhone X$'}},{ productPrice:{$gte:1600}}]}).toArray()
  console.log(result)
}
...
/*
Ты - мажор и хочешь дорогую покупку. или Айфон X) Найди все товары, которые либо точно 'IPhone X', либо дороже или равны 1600
Если всё ок, ты найдёшь Phone X и Asus ZenBook
В качестве ответа укажите этот объект-фильтр (можно писать его с форматированием в несколько строк)
*/
